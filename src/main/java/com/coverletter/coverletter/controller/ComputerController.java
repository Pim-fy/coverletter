...
public class ComputerController {
    ...
    public ComputerDto.CreateResponse createResponse(@PathVariable Long userId, @RequestBody @Valid ComputerDto.CreateRequest request) {
        return computerService.createResponse(userId, request);
    }

    @GetMapping("/{userId}")        // 컴퓨터 능력 정보 조회
    public ComputerDto.ReadResponse readResponse(@PathVariable Long userId) {
        return computerService.readResponse(userId);
    }

    @PutMapping("/{userId}")        // 컴퓨터 능력 정보 수정
    public ComputerDto.UpdateResponse updateResponse(@PathVariable Long userId, @RequestBody @Valid ComputerDto.UpdateRequest request) {
        return computerService.updateResponse(userId, request);
    }

    @DeleteMapping("/{userId}/{computerId}")      // 컴퓨터 능력 정보 삭제
    public ComputerDto.DeleteResponse deleteResponse(@PathVariable Long userId, @PathVariable Long computerId) {
